...
st.sidebar.header('Choose a watershed')
watershed_select = st.sidebar.selectbox('Select', watersheds)
...
from_date = st.sidebar.date_input('From:', value=start_date)

def show_success():
    st.toast('Proper date range', icon="✅")

to_date = st.sidebar.date_input('To:', value=end_date, on_change=show_success, help='Warning: choose a date longer than the start date')

if from_date > to_date:
    st.sidebar.error('Warning: start date greater than end date')
    df2 = df2
...
st.subheader(f'Watershed: {watershed_select} | Period: **{from_date.strftime('%d/%b/%Y')}** to **{to_date.strftime('%d/%b/%Y')}**')
...
with col1:
    ...
    st.metric('Max streamflow (m$^{3}$/s):', value=max_streamflow)
    st.metric('Min streamflow (m$^{3}$/s):', value=min_streamflow)
    st.metric('Mean streamflow (m$^{3}$/s):', value=mean_streamflow)
...
with col2:
    ...
    st.metric('Max baseflow (m$^{3}$/s):', value=max_baseflow)
    st.metric('Min baseflow (m$^{3}$/s):', value=min_baseflow)
    st.metric('Mean baseflow (m$^{3}$/s):', value=mean_baseflow)
...
with col3:
    ...
    st.metric('Area (km$^{2}$)', value=area_bacia)
    ...
    st.metric('Data period (years)', value=diff)
...
with col5:
    ...
    fig.update_yaxes(title_text='<b>Flow (m<sup>3</sup> s<sup>-1</sup>)</b>', title_font={"size": 14}, range=[0, None])
    fig.update_xaxes(title_text='<b>Time</b>', title_font={"size": 14})
    fig.update_layout(template='plotly', legend_title='<b>Legend</b>')    # plotly_dark    simple_white
...
with col6:
    ...
    st.dataframe(df3, width=250, height=300, hide_index=True, column_order=('Date', 'streamflow', 'baseflow'), use_container_width=True, column_config={'Date': st.column_config.DatetimeColumn('Date', format="DD/MMM/YYYY"),
                                                                                                                                                        'streamflow': st.column_config.NumberColumn("Streamflow", help="Streamflow series", format='%.1f'),
                                                                                                                                                        'baseflow': st.column_config.NumberColumn("Baseflow", help="Baseflow series", format='%.1f')})
